...
pub async fn run_mcp_stdio(store: Arc<Mutex<Store>>) {
    ...
    while let Ok(Some(raw_line)) = lines.next_line().await {
        ...
        let response = match method {
            ...
            "tools/call" => {
                ...
                json_rpc_response(id, handle_tools_call(&params, &store))
            }
...
fn handle_tools_call(params: &serde_json::Value, store: &Arc<Mutex<Store>>) -> serde_json::Value {
    let name = params.get("name").and_then(|v| v.as_str()).unwrap_or("");
    let arguments = params.get("arguments").cloned().unwrap_or(json!({}));
    let result = execute_tool(store, name, &arguments);
...
mod tests {
    ...
    fn initialize_response() {
        ...
        assert_eq!(resp["serverInfo"]["name"], "photisnadi");
    }

    #[test]
    fn tools_list_returns_six() {
    ...
    fn tools_call_unknown() {
        let store = Arc::new(Mutex::new(Store::new_in_memory().unwrap()));
        let resp = handle_tools_call(&json!({"name": "unknown"}), &store);
        assert!(resp["isError"].as_bool().unwrap());
    }

    #[test]
    fn tools_call_list_tasks() {
        let store = Arc::new(Mutex::new(Store::new_in_memory().unwrap()));
